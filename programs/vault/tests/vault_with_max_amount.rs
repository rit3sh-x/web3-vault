...
#[test]
fn test_deposit_fails_when_exceeds_max_amount() {
    let (mut svm, user_authority) = setup();
    ...
    let deposit_amount = max_amount.checked_add(1000).unwrap();
    ...
    let message = Message::new(&[deposit_ix], Some(&user_authority.pubkey()));
